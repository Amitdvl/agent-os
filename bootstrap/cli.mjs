...
const TOOL_CONTRACTS = {
  ...
  discrawl: { root: "<tool-owned Discord archive>", preflight: ["discrawl --help"], reads: ["discrawl sync --full", "discrawl wiretap --full-cache", "discrawl search <query>", "discrawl dms --list"], writes: ["Supported discrawl send only with an exact channel and message"], limits: "Wiretap may import only classifiable messages already cached by the user's Desktop profile, including direct messages; it never fetches user-account history remotely.", fix: "Use the tool's documented account setup; do not expose a bot token." },
...
const TOOL_DETAILS = {
  ...
  discrawl: "Run a full sync before archive answers and after supported writes. Use wiretap --full-cache for locally cached DMs; never combine account profiles or expose private messages beyond the request.",
