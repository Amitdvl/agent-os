...
const TOOL_CONTRACTS = {
  ...
  remindctl: { root: "<Apple Reminders database>", preflight: ["remindctl --help"], reads: ["remindctl list"], writes: ["Create/update/complete/delete only with exact list, reminder, date, and action", "Urgent means an alarm, not priority; when a due date/time is provided, create an alarm at that same time by default and leave priority as none unless explicitly requested"], limits: "Results can lag native state and access requires macOS permission.", fix: "Grant Reminders access in System Settings, then verify in the native app." },
...
const TOOL_DETAILS = {
  ...
  remindctl: "Use JSON and read back writes. Urgent is the alarm setting, not priority: when a due date/time is provided, set the alarm to that same time by default and keep priority none unless explicitly requested. Native Reminders UI is freshest for Today/current/subtasks; never use it for calendar events.",
