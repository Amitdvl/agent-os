...
const TOOL_CONTRACTS = {
  asc: { root: "<user-owned ASC profile/config and system keychain>", preflight: ["asc version", "asc --help", "asc auth status --validate", "asc auth doctor"], reads: ["asc apps list --output json"], writes: ["Uploads, TestFlight distribution/invitations, review submissions, releases, pricing, deletion, and account changes only with exact app/resource IDs and requested action", "Inspect command help, use a supported dry run when available, and read back remote state after each write"], limits: "Apple developer account permissions and API coverage apply. Apple Ads, StoreKit credentials, and web sessions are separate integrations; do not infer authorization for them.", fix: "Use supported auth status/doctor with sanitized output; resolve the intended account through Agent Vault or user-owned login. Never inspect keychain/config contents or enable debug output to recover credentials." },
...
const TOOL_DETAILS = {
  asc: "App Store Connect CLI upstream: https://github.com/rorkai/App-Store-Connect-CLI. Reviewed install route: brew install asc; installation is deliberate and no version is pinned yet. Use asc version and command help before relying on flags. Prefer explicit JSON and bounded queries supported by current help; do not paginate an entire account by default. Check the intended profile and stable app ID before writes. Agent Vault requirements: ASC_KEY_ID, ASC_ISSUER_ID for team keys, and one of ASC_PRIVATE_KEY_PATH, ASC_PRIVATE_KEY, or ASC_PRIVATE_KEY_B64. ASC_KEY_TYPE selects team or individual; individual keys omit issuer ID. Inject credentials through the vault, never inline secret values or copy .p8 files into repositories. ASC_PROFILE selects stored credentials and changes environment resolution; avoid mixing sources, and use ASC_STRICT_AUTH=true when appropriate. Never automatically bypass keychain or create repo-local credential config. Upstream enables command telemetry by default; ASC_TELEMETRY_DISABLED=1 or DO_NOT_TRACK=1 disables it.",
