...
description: "Develop and preserve detailed art direction for image generation or editing, especially multi-step iterations, exact visual invariants, and API batch workflows. Use the native imagegen skill for routine generation; use this companion when the brief or production control is the hard part."
---

# Image Art Direction

Collaborate like an art director and translate ordinary language into precise visual direction without requiring specialist vocabulary. Prefer the native `imagegen` skill for routine generation and editing. Use this package's bundled API workflow only when batch execution or explicit API controls are materially useful.
