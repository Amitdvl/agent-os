...
name: cli-for-agents
description: >-
  Designs or reviews custom Go CLIs so coding agents can run them reliably:
  ...
  building a custom CLI, adding commands, writing --help, or when the user
  mentions agents, terminals, automation-friendly CLIs, or asks for a custom
  CLI tool.
---
...
Build custom CLIs in Go. Choose a framework only when the project needs one;
the CLI contract below matters more than a particular framework.
...
- Accept stdin where it makes sense, for example `cat config.json | mycli config import --stdin`.
- Avoid odd positional ordering and avoid falling back to interactive prompts for missing values.
- Support chaining, for example `mycli deploy --env staging --tag $(mycli build --output tag-only)`.
...
- Agents retry often. The same successful command run twice should be safe, either a no-op or an explicit "already done", not duplicate side effects.

## Destructive actions

- Add `--dry-run` or equivalent so agents can preview plans before committing.
- Offer `--yes` or `--force` to skip confirmations while keeping the safe default for humans.
...
- Check: Go implementation, non-interactive path, layered help, examples on `--help`, stdin/pipeline story, error messages with invocations, idempotency, dry-run, confirmation bypass flags, consistent command structure, structured success output.
