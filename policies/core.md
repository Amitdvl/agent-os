...
- For macOS build and test work, prefer a stable, project-local ignored output or derived-data directory when the tool supports it. Create a per-run output root only when isolation or retained evidence requires it.
- Before completing a build or test task that produces application bundles or other sizable generated artifacts, inventory those artifacts and identify the active or deployed bundle separately. Keep source, untracked user work, active or deployed apps, requested evidence, and unrelated worktrees.
