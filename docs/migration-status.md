...
| Goal and orchestration contracts | `$CODEX_HOME/skills/{goal-prompt,orchestration}` and `$CODEX_HOME/AGENTS.md` | active dependency | **portable source:** `skills/{goal-prompt,orchestration}/SKILL.md`, `policies/orchestration.md`, and the core pack. Both host skills are compared byte-for-byte by twin audit. |
