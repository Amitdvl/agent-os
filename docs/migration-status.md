...
| Task orchestration | `$CODEX_HOME/skills/orchestration` | active dependency | **portable source:** `skills/orchestration/SKILL.md`, `policies/orchestration.md`, and the core pack. The host skill is compared byte-for-byte by twin audit. |
| Image generation and art direction | `$CODEX_HOME/skills/imagegen` | active dependency | **portable source:** `skills/imagegen/`, `manifest/skills.json`, and the core pack. The behavioral contract is compared byte-for-byte by twin audit; the private source lexicon and host PNG icon are explicit reference-only exclusions. |
...
| Imagegen workflow | `$CODEX_HOME/skills/imagegen` | Agent OS portable skill package | `skills/imagegen/`, installed to both supported hosts from the core pack; private source material remains reference-only. |
