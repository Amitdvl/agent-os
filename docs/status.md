...
Current coverage includes manifest validation, core-only safe setup, seven default core commands, full two-host deployment, an explicit-only book skill whose complete fixture-backed test suite installs to both hosts, a Pamphlet command and skill for researched 300–1,000-word PDF/Markdown knowledge sprints, central registry rendering, 24 tool templates including a pinned read-only Telgo channel-digest contract, guarded Silicon code-image generation, Pandoc conversion, and EPUBCheck validation contracts, host symlink plans, generated allow rules, install dry-runs, fixture-only SOPS/age vault initialization/validation, a separate Codex command-cutover/rollback transaction, status/doctor distinctions, drift/conflict refusal, safe uninstall, and a fixture-backed production-repo baseline helper that safely foundations an empty directory without fabricating application CI.
