...
Current coverage includes manifest validation, core-only safe setup, seven default core commands, the complete `$pamphlet` and `$book` packages, the production repository baseline helper, quiet fallacy checking, full two-host deployment, central registry rendering, 20 tool templates, host symlink plans, generated allow rules, install dry-runs, fixture-only SOPS/age vault initialization/validation, a separate Codex command-cutover/rollback transaction, status/doctor distinctions, drift/conflict refusal, and safe uninstall.
