...
test("portable command and goal contracts retain their required workflow sections", async () => {
  const required = {
    ...
    "skills/goal-prompt/SKILL.md": ["3,800", "at most three", "visual work", "metric gaming", "Progress reporting", "`/goal` is an orchestration trigger", "beginning of the goal", "Never claim a model or delegation occurred", "worker output as evidence"],
    "skills/orchestration/SKILL.md": ["## Activation", "## Lead Contract", "## Assignments", "## Coordination Rules", "## Completion Gate", "every `/goal` activates orchestration", "multiple files", "Never claim a model or delegation occurred", "worker output as evidence", "silently redirect"],
  };
